...
> It exists so this project has its own citable GitHub identity, its own
> release history, and a stable home for the rendered `output/` artifacts
> (the actual PDF/PPTX decks and the per-slide standalone pages every slide's
> QR code points to) — matching a real project's own citable code, not a
> copy pasted out of context. Relative links below that point at sibling
> exemplars (`../template_template/`, `../../AGENTS.md`) refer to that
> monorepo's own structure and won't resolve from this checkout.
...
- **Rendering primitives (generic, new to this repo):** `infrastructure/rendering/slide_deck.py` (ReportLab PDF) and `infrastructure/rendering/pptx_deck.py` (python-pptx, opt-in) — both consume the same `DeckContent` model and the same font-size constants (imported, not re-declared), so a PDF and a PPTX built from identical content carry identical slide counts, text, and type sizing. `infrastructure/rendering/mermaid_figure.py` renders standalone Mermaid diagrams to PNG for embedding.
- **Theme:** black + white base plus three configurable highlight colors (`infrastructure.rendering.slide_deck.DeckTheme`), set under `manuscript/config.yaml`'s `deck.theme` block. Ships monochrome red by default; `config.yaml.example` demonstrates a distinct multi-accent palette.
